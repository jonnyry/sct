...
One JSON object per line, sorted by concept SCTID (ascending numeric order). The file is a valid NDJSON document — every line is a standalone JSON object.
...
| `schema_version` | integer | Artefact schema version (currently `1`) |

---

## Querying with standard tools

The NDJSON artefact is designed to be queried directly with `jq` without any custom tooling.
...
# Check integrity — line count should match expected concept total
wc -l snomed.ndjson
```
...
Given the same RF2 Snapshot directory and the same `--locale`, `sct ndjson` always produces byte-for-byte identical output:
